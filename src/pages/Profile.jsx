...
export const Profile = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    getPosts().then(posts => {
      const filteredPosts = posts.filter(post => post.author.id === currentUser.neighbor_id);
      setUserPosts(filteredPosts);
    });

    // Fetch all comments and filter them by the current user's ID
    getComments().then(comments => {
      const filteredComments = comments.filter(comment => comment.author.id === currentUser.neighbor_id);
    ...
    <div className="flex w-full my-6">
      ...
      <div className="flex-3 p-8 bg-white rounded-lg shadow-lg flex">
        <div className="w-1/2 pr-4">
          ...
          {userPosts.map((post) => (
            <Link to={`/posts/${post.id}`} key={post.id} className="block p-4 rounded-lg shadow-md bg-green-100 mb-4">
        ...
        <div className="w-1/2 pl-4">
          ...
          {userComments.map((comment, index) => (
            <div key={index} className="bg-gray-100 p-2 my-2 rounded cursor-pointer shadow-md" onClick={() => navigate(`/posts/${comment.post.id}`)}>
              <p>{comment.content}</p>
              <p className="text-sm text-gray-600">On post: {comment.post.title}</p>
            </div>
          ))}
        </div>
      </div>
